...
local ICON_COP = "Interface\\Icons\\Ability_Priest_ClarityOfPurpose"
...
local ICONTEXTURES = {
	...
	['cop'] = "Interface\\Icons\\Ability_Priest_ClarityOfPurpose",
...
-- local data
local settings, settings_coh, settings_poh, settings_cop, settings_ch, settings_hn, settings_hr, settings_wg, settings_debug
local num_enabled = 0
	...
	for i = 1, UF[frameHeader]:GetNumChildren() do
		...
		for j = 1, maxChildren do
			...
			if frame and UnitExists(frame.unit) then
				unitIDToFrameIndex[frame.unit] = {i,j,frame}
			end
		end
	end
...
function EPA:RefreshCoH()
	...
	if coh_best_uid and overheal <= settings_coh.threshold then
		if coh_prev_best.uid ~= coh_best_uid or coh_prev_best.guid ~= coh_best_pdata.guid or coh_prev_best.amount ~= coh_best_amount then
			EPA:SendStatusGained(coh_best_uid, 'coh', min(coh_best_targets, coh_targets), overheal)
		end
...
function EPA:RefreshHR()
	...
	-- send status to best
	local overheal = math.min((1 - hr_best_amount/(hr_avg * 4)) * 100, 100) -- 1 target at 100%, 6 targets at 50% == 4
	...
	if hr_best_uid and overheal <= settings_hr.threshold then
		if hr_prev_best.uid ~= hr_best_uid or hr_prev_best.guid ~= hr_best_pdata.guid or hr_prev_best.amount ~= hr_best_amount or settings_hr.threshold == 100 then
...
function EPA:SendStatusGained(uid,icon,numtargets,overheal)
	local group, index, parent = unitIDToFrameIndex(uid)
	if group and index and parent then
